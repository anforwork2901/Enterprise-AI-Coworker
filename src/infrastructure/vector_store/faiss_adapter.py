"""
FAISSAdapter — LangChain FAISS implementation of VectorStorePort.

Provides persona-scoped knowledge retrieval for RAG-augmented NPC responses.
Each persona gets its own FAISS index namespace for clean knowledge separation.
"""
import logging
import os
...
class FAISSAdapter(VectorStorePort):
    """
    Concrete VectorStorePort using LangChain's FAISS integration.
    ...
    def __init__(self, api_key: str, embedding_model: str, index_path: str) -> None:
        ...
        logger.info("FAISSAdapter initialized at '%s'", index_path)
